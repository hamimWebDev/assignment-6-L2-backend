...
const OptionalAuth = (...requiredRoles: IUserRole[]) => {
  return catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const token = req.headers['authorization']?.split(' ')[1]; // Get token if available

    // If no token is provided, proceed without authentication
    if (!token) {
      (req as any).user = null; // Set req.user to null if no token is provided
    ...
    try {
      ...
      if (!user) {
        (req as any).user = null; // Set user to null if not found
        return next(); // Allow the request to continue
      }
      ...
      if (requiredRoles.length > 0 && !requiredRoles.includes(decoded.role as IUserRole)) {
        (req as any).user = null; // Reset user if they don't have the required role
        return next(); // Allow the request to continue
      }

      next(); // Proceed to the next middleware or route handler
    } catch (err) {
      (req as any).user = null; // Set user to null if token verification fails
      return next(); // Allow the request to continue
    }
  });
};
