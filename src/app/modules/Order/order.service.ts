...
const createOrder = async (user: JwtPayload, payload: Partial<IOrder>) => {
  ...
  const orderData: Partial<IOrder> = {
    ...
    endDate:
      payload.endDate ||
      moment().add(payload.subscriptionDuration, 'months').toDate(), // Calculate endDate based on subscriptionDuration
  ...
  const paymentData = {
    ...
    endDate:
      payload.endDate ||
      moment().add(payload?.subscriptionDuration, 'months').toDate(),
  }
